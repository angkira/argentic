...
async def test_connect_event():
    ...
    with patch.object(sio, "emit", new_callable=AsyncMock) as mock_emit:
        ...
        mock_emit.assert_called_once_with("connection_status", {"status": "connected"}, room=sid)
...
async def test_subscribe_event():
    ...
    with patch.object(sio, "emit", new_callable=AsyncMock) as mock_emit:
        ...
        mock_emit.assert_called_once_with("subscribed", {"topic": topic}, room=sid)
...
async def test_unsubscribe_event():
    ...
    with patch.object(sio, "emit", new_callable=AsyncMock) as mock_emit:
        ...
        mock_emit.assert_called_once_with("unsubscribed", {"topic": topic}, room=sid)
